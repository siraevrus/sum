...
```
**Описание:** Архивирует компанию вместо удаления. Архивированная компания не отображается в обычных списках, но сохраняет все данные и связи.
...
```json
{
    ...
    "data": {
        ...
        "is_archived": true,
        "archived_at": "2025-08-29T10:30:00.000000Z"
    }
}
```

**Ответ при ошибке (компания уже архивирована):**
```json
{
    "success": false,
...
```
**Описание:** Восстанавливает компанию из архива, делая её снова активной.
...
**Ответ при ошибке (компания не архивирована):**
```json
{
    "success": false,
...
```
**Важно:** Компания может быть удалена только если у неё нет связанных складов или сотрудников.

**Ответ при ошибке (есть зависимости):**
```json
{
    "success": false,
    ...
    "details": {
        "warehouses_count": 2,
        "employees_count": 5,
        "suggestion": "Используйте POST /api/companies/14/archive для архивирования"
    }
}
```
...
## Решение проблемы с удалением компаний

Если при попытке удалить компанию вы получаете ошибку 400 с сообщением о связанных записях, используйте **архивирование** вместо удаления:

1. **Архивируйте компанию:**
   ```
   POST /api/companies/{id}/archive
   ```

2. **При необходимости восстановите:**
   ```
   POST /api/companies/{id}/restore
   ```

Архивирование позволяет:
- Скрыть компанию из активных списков
- Сохранить все данные и связи
- Восстановить компанию в будущем
